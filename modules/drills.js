// modules/drills.js — Drill library: 24 drills, category metadata, templates
...
export const DRILLS = [
  ...
  },
  {
    ...
    category: 'evasion',
    duration: 5,
    description: 'Leader runs at 70% speed making cuts and moves, others mimic in a line.',
  ...
  // ── DEFENSE ─────────────────────────────────────────────────
  {
    ...
    category: 'defense',
    duration: 8,
    description: "Defenders read coach's eyes/arm, break on the ball.",
...
// ── Custom drills registry (populated from localStorage at load) ──
export const CUSTOM_DRILLS = [];

// Get drill by id (searches built-in + custom)
export function getDrillById(id) {
  return DRILLS.find(d => d.id === id) || CUSTOM_DRILLS.find(d => d.id === id) || null;
}

// All drills (built-in + custom)
export function getAllDrills() {
  return [...DRILLS, ...CUSTOM_DRILLS];
}
